clear; clc;
multithreading()
% todo: pareto optimization => use gamultiobj()

% f = @(x)DDP_erods_renyi(x(1));
% 
% nvars = 1;
% A = [];
% b = [];
% Aeq = [];
% beq = [];
% lb = [0.01];
% ub = [1];
% nonlcon = [];
% intcon  = [];
% % x = gamultiobj(f,nvars,A,b,Aeq,beq,lb,ub,nonlcon,intcon);
% x = paretosearch(f,nvars,A,b,Aeq,beq,lb,ub,nonlcon);
% % x = [0.011235];
% 
% figure();
% for i = x'
%     disp(i)
%     subplot(2,1,1)
%     hold on
%     [cost, size] = DDP_erods_renyi(i(1));
%     plot(cost,size,'o')
%     title('Pareto Points')
%     xlabel("cost")
%     ylabel("100-size")
%     xlim([0 1])
%     ylim([0 1])
%     hold off
% 
%     subplot(2,1,2)
%     hold on
%     plot(i(1),i(1),'o')
%     title('Parameters 1')
%     xlabel("p")
%     ylabel("p")
%     xlim([0 1])
%     ylim([0 1])
%     hold off
% end
% 
% f = @(x)DDP_watts_strogatz(1, x(1));
% 
% nvars = 1;
% A = [];
% b = [];
% Aeq = [];
% beq = [];
% % lb = [1 0];
% % ub = [3 1];
% lb = [0];
% ub = [1];
% nonlcon = [];
% % intcon  = [1];
% % x = gamultiobj(f,nvars,A,b,Aeq,beq,lb,ub,nonlcon,intcon);
% x = paretosearch(f,nvars,A,b,Aeq,beq,lb,ub,nonlcon);
% % x = [1 x(1)];
% % % x = [1.0000 0.8872];
% 
% figure();
% for i = x'
%     disp(i)
%     subplot(2,1,1)
%     hold on
%     [cost, triv] = DDP_watts_strogatz(i(1), i(2));
%     plot(cost,triv,'o')
%     title('Pareto Points')
%     xlabel("cost")
%     ylabel("triv.")
%     xlim([0 1])
%     ylim([0 1])
%     hold off
% 
%     subplot(2,1,2)
%     hold on
%     plot(i(1),i(2),'o')
%     title('Parameters 1')
%     xlabel("K/2")
%     ylabel("beta")
%     xlim([0 1])
%     ylim([0 1])
%     hold off
% end

% f = @(x)DDP_sfg(x(1), x(2), x(3));
% 
% nvars = 3;
% A = [];
% b = [];
% Aeq = [1 1 1];
% beq = 1;
% lb = [0.01 0.01 0.01];
% ub = [1 1 1];
% nonlcon = [];
% intcon  = [];
% % x = gamultiobj(f,nvars,A,b,Aeq,beq,lb,ub,nonlcon,intcon);
% 
% x = paretosearch(f,nvars,A,b,Aeq,beq,lb,ub,nonlcon);
% x = [x(1) x(2) x(1)];
% % x = [0.4950    0.0100    0.4950; 0.3816    0.2367    0.3816];
% 
% figure();
% for i = x'
%     disp(i)
%     disp(sum(i))
%     subplot(3,1,1)
%     hold on
%     [cost, triv] = DDP_sfg(i(1), i(2), i(3), "sample_size", 100);
%     plot(cost,triv,'o')
%     title('Pareto Points')
%     xlabel("cost")
%     ylabel("triv.")
%     xlim([0 1])
%     ylim([0 1])
%     hold off
% 
%     subplot(3,1,2)
%     hold on
%     plot(i(1),i(2),'o')
%     title('Parameters 1')
%     xlabel("alpha")
%     ylabel("beta")
%     xlim([0 1])
%     ylim([0 1])
%     hold off
% 
%     subplot(3,1,3)
%     hold on
%     plot(i(1),i(3),'o')
%     title('Parameters 2')
%     xlabel("alpha")
%     ylabel("gamma")
%     xlim([0 1])
%     ylim([0 1])
%     hold off
% end

cost = [];
triv = [];
i_list = [];

for i = 0:0.05:1
    disp(i)
    [cost(end+1), triv(end+1)] = DDP_watts_strogatz(1, i);
    i_list(end+1) = i;
end

figure();
subplot(2,1,1)
hold on

plot(cost,triv,'k-o')
plot(cost(1),triv(1),'r*')
title('Pareto Points')
xlabel("cost")
ylabel("triv.")
% xlim([0 1])
% ylim([0 1])
hold off

subplot(2,1,2)
hold on
plot(i_list,'k-o')
plot(i_list(1),'r*')
title('Parameters 1')
xlabel("K/2")
ylabel("beta")
% xlim([0 1])
% ylim([0 1])
hold off


function [cost, size] = DDP_erods_renyi(p)
    disp("p = " + p)
    fract_targ = 0.1;
    fract_dist = 0.1;
    n = 100;
    params = num2cell([n, p]);
    [costs, ~, ~, sizes] = run_test( ...
        @erdos_renyi, ...
        params, ...
        "sample_size", 8, ...
        "fraction_targets", fract_targ, ...
        "fraction_disturbances", fract_dist, ...
        "ddp", "state_feedback" ...
        );
    cost = mean(costs);
    size = mean(100-sizes);
    disp("cost = " + cost + ", 100-size = " + size)
end

function [cost, trivial] = DDP_watts_strogatz(k, beta)
    disp("k = " + k)
    disp("beta = " + beta)
    fract_targ = 0.1;
    fract_dist = 0.1;
    n = 100;
    params = num2cell([n, k, beta]);
    [costs, ~, trivials] = run_test( ...
        @watts_strogatz, ...
        params, ...
        "sample_size", 1000, ...
        "fraction_targets", fract_targ, ...
        "fraction_disturbances", fract_dist, ...
        "ddp", "state_feedback" ...
        );
    cost = mean(costs);
    trivial = mean(trivials);
    disp("cost = " + cost + ", triv. = " + trivial)
end

function [cost, trivial] = DDP_sfg(alpha, beta, gamma)
    disp("alpha = " + alpha + ", beta = " + beta + ", gamma = " + gamma)
    fract_targ = 0.1;
    fract_dist = 0.1;
    n = 100;
    params = num2cell([n, alpha, beta, gamma, 1, 1]);
    [costs, ~, trivials] = run_test( ...
        @sfg, ...
        params, ...
        "sample_size", 6, ...
        "fraction_targets", fract_targ, ...
        "fraction_disturbances", fract_dist, ...
        "ddp", "state_feedback" ...
        );
    cost = mean(costs);
    trivial = mean(trivials);
    disp("cost = " + cost + ", triv. = " + trivial)
end
